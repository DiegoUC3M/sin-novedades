...
public final class TabDetectorTest {
    ...
    public static void main(String[] args) {
        ...
        TabDetector.Navigation nav=TabDetector.navigation(normal(),SCREEN,1,false);
        expect(!nav.updatesOpen() && nav.selected.isEmpty(),"An unreported selection must not hide another page");
        List<TabDetector.Tab> selected=normal();
        TabDetector.Tab updates=selected.get(1);
        selected.set(1,new TabDetector.Tab(updates.kind,updates.hit,updates.bar,true,true));
        nav=TabDetector.navigation(selected,SCREEN,1,false);
        expect(nav.updatesOpen(),"Selected Updates must hide its content");
        expect(nav.content.same(b(0,24,400,800)),"Black page covers content up to the actual footer");
        expect(nav.content.bottom<=nav.target.top,"Footer remains reachable to leave the black page");
        expect(TabDetector.navigation(selected,SCREEN,1,true)==null,"No black page over the chat editor");
        TabDetector.Tab chats=selected.get(0);
        selected.set(0,new TabDetector.Tab(chats.kind,chats.hit,chats.bar,true,true));
        expect(!TabDetector.navigation(selected,SCREEN,1,false).updatesOpen(),"Conflicting selection must not trap the user");
        selected.set(1,updates);
        nav=TabDetector.navigation(selected,SCREEN,1,false);
        expect(!nav.updatesOpen() && nav.selected.equals("chats"),"Selecting Chats removes the black page");
        selected=normal();
        selected.add(new TabDetector.Tab(updates.kind,updates.hit,updates.bar,true,true));
        expect(TabDetector.navigation(selected,SCREEN,1,false).updatesOpen(),"Selected child survives duplicate label/container merging");
        selected=normal();
        selected.add(new TabDetector.Tab("updates",b(100,405,200,475),b(0,400,400,480),true,true));
        expect(!TabDetector.navigation(selected,SCREEN,1,false).updatesOpen(),"Selected content outside the footer is ignored");
        selected=new ArrayList<>();
        for (TabDetector.Tab t:normal()) selected.add(new TabDetector.Tab(t.kind,t.hit,null,true,t.kind.equals("updates")));
        nav=TabDetector.navigation(selected,SCREEN,1,false);
        expect(nav.updatesOpen() && nav.content.bottom==805,"Flattened semantic tabs leave their entire row accessible");
        selected=new ArrayList<>();
        for (TabDetector.Tab t:resized) selected.add(new TabDetector.Tab(t.kind,t.hit,t.bar,true,t.kind.equals("updates")));
        nav=TabDetector.navigation(selected,b(0,72,1200,2700),3,false);
        expect(nav.content.same(b(0,72,1200,2400)),"Black page uses real high-density bounds");
        expect(TabDetector.selectedDescription("seleccionada"),"Spanish state description");
        expect(TabDetector.selectedDescription("Novedades, seleccionada, pestaña 2 de 4"),"Selection in Spanish tab description");
        expect(TabDetector.selectedDescription("Updates\nSelected, tab 2 of 4"),"Selection in multiline English metadata");
        expect(!TabDetector.selectedDescription("Novedades, no seleccionada, pestaña 2 de 4"),"Negative Spanish selection");
        expect(!TabDetector.selectedDescription("Updates, not selected"),"Negative English selection");
        expect(!TabDetector.selectedDescription("unselected"),"Unselected is not selected");
        expect(!TabDetector.selectedDescription("Mensaje seleccionado"),"Arbitrary selected content is not a tab");
        expect(!TabDetector.selectedDescription("Novedades"),"A label alone does not prove selection");
